...
static void triangle(int a, int b, int c, std::vector<Vertex> &vertices) {
	...
	for (int z = 0; z < range; z++) {
		for (int y = 0; y < range; y++) {
			for (int x = 0; x < range; x++) {
				...
				std::array<int, 12> edge_indices;

				do_edge(0, vs[0], vs[1], 0, Vec3f(x, y, z), edge_indices, vertices);
				do_edge(1, vs[2], vs[3], 0, Vec3f(x, y + 1, z), edge_indices, vertices);
				do_edge(2, vs[4], vs[5], 0, Vec3f(x, y, z + 1), edge_indices, vertices);
				do_edge(3, vs[6], vs[7], 0, Vec3f(x, y + 1, z + 1), edge_indices, vertices);

				do_edge(4, vs[0], vs[2], 1, Vec3f(x, y, z), edge_indices, vertices);
				do_edge(5, vs[1], vs[3], 1, Vec3f(x + 1, y, z), edge_indices, vertices);
				do_edge(6, vs[4], vs[6], 1, Vec3f(x, y, z + 1), edge_indices, vertices);
				do_edge(7, vs[5], vs[7], 1, Vec3f(x + 1, y, z + 1), edge_indices, vertices);

				do_edge(8, vs[0], vs[4], 2, Vec3f(x, y, z), edge_indices, vertices);
				do_edge(9, vs[1], vs[5], 2, Vec3f(x + 1, y, z), edge_indices, vertices);
				do_edge(10, vs[2], vs[6], 2, Vec3f(x, y + 1, z), edge_indices, vertices);
				do_edge(11, vs[3], vs[7], 2, Vec3f(x + 1, y + 1, z), edge_indices, vertices);
				...
				for (int i = 0; i < n_triangles; i++) {
					triangle(
						...
						indices[index_base + i * 3 + 2], vertices);
				}
			}
		}
	}
